 ...
 */
void get_ion_rate(double *y1H, double *y1He, double *fracflux, double *dy1H, double *dy1He, double *dEH, double *dEHI, double *dEHII, double *dEHeI, double *dEHeII, double *Te, double *THI, double *THII, double *THeI, double *THeII, double *tauHIIe) {
  ...
  for(j=0;j<NGRID;j++){
      if (j>0){
          ...
          tauHIIe[j] = 6.97e-16 *(1.-y1H[j]+ABUND_HE*(1-y1He[j]))/pow(Te[j],3/2) * (Te[j]-THII[j]);
          //dEH[j] -= tauHIIe;
          //dEHII[j] += tauHIIe;
      }
  }
...
int main(int argc, char **argv) {
  ...
  for(istep=0;istep<NTIMESTEP;istep++) {
    ...
    get_ion_rate(y1H,y1He,fracflux, dy1H,dy1He,dEH,dEHI,dEHII,dEHeI,dEHeII,Te,THI,THII,THeI,THeII,tauHIIe);
    for(j=0;j<NGRID;j++)
  ...
  for(j=0; j<NGRID; j++) {
    printf("%4ld %11.5lE %8.6lf %8.6lf %8.6lf %7.1lf %8.6lf %7.1lf %8.100lf %8.100lf\n",
