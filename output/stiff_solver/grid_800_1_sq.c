 ...
 /* 
  for(j=0;j<NGRID;j++){
      if (istep>-1){
          ...
          tauHIIe[j] = 6.97e-16 *(1.-y1H[j])/pow(Te[j],3/2) * (Te[j]-THII[j]);
          
          if (dEH[j] - tauHIIe[j] > 0 && dEHII[j] + tauHIIe[j] > 0) {
            dEH[j] -= tauHIIe[j];
            dEHII[j] += tauHIIe[j];
          } else if (dEH[j] - tauHIIe[j] < 0 && dEHII[j] + tauHIIe[j] > 0) {
            dEHII[j] += dEH[j];
            dEH[j] = 0;
          } else if (dEH[j] - tauHIIe[j] > 0 && dEHII[j] + tauHIIe[j] < 0) {
            dEH[j] -= dEHII[j];
...
int main(int argc, char **argv) {
  ...
  for(istep=0;istep<NTIMESTEP;istep++) {
    ...
    for(j=0;j<NGRID;j++) {
      ...
      M[0][0] = (1.+7.335e-11*(y1H[j])*(1.-y1H[j])*DTIMESTEP/(pow(Te[j],3/2)+pow(THII[j],3/2)))/(1.+1.467e-10*(1.-y1H[j])*(y1H[j])*DTIMESTEP/(pow(Te[j],3/2)+pow(THII[j],3/2)));
      M[0][1] = 0.5*(1.-y1H[j])*(y1H[j])*DTIMESTEP/(6.816e9*pow(Te[j],3/2)+6.816e9*pow(THII[j],3/2)+(1-y1H[j])*(y1H[j])*DTIMESTEP);
      M[1][0] = M[0][1];
      M[1][1] = M[0][0];
...
*/
    double Tej = Te[j];
    double THIIj = THII[j];
    Te[j] = M[0][0]*Tej + M[0][1]*THIIj;
    THII[j] = M[1][0]*Tej + M[1][1]*THIIj;

    EH[j] = Te[j]*1.5*(2.-y1H[j]+ABUND_HE*(2.-y1He[j]))/RYD_K;
    EHII[j] = THII[j]*1.5*(2.-y1H[j])/RYD_K;
