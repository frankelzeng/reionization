...
int main(int argc, char **argv) {
  ...
  double nuTeTHII, nuTeTHI, nuTHIITHI, nuTeTHeI, nuTHIITHeI, nuTHITHeI;
  ...
  for(istep=0;istep<NTIMESTEP;istep++) {
    if (istep==0) {
      for(j=0;j<NGRID;j++) {
        ...
        Te[j] = EH[j]/1.5/(1.-y1H[j]+ABUND_HE*(2.- 2*y1He[j]))*RYD_K;
...
// Energy transferring function using temeprature transformation
     ...
     double THIj = THI[j];
     double THeIj = THeI[j];
     Te[j]   = I[0][0]*Tej + I[0][1]*THIIj + I[0][2]*THIj + I[0][3]*THeIj;
     THII[j] = I[1][0]*Tej + I[1][1]*THIIj + I[1][2]*THIj + I[1][2]*THeIj;
     THI[j]  = I[2][0]*Tej + I[2][1]*THIIj + I[2][2]*THIj + I[2][3]*THeIj;
     THeI[j] = I[3][0]*Tej + I[3][1]*THIIj + I[3][2]*THIj + I[3][3]*THeIj;

     EH[j] = Te[j]*1.5*(1.-y1H[j]+ABUND_HE*(2.- 2*y1He[j]))/RYD_K;
...
*/
  ...
  printf("Timestep=%7d\n", NTIMESTEP);
  printf("j, (j+.5)*DNHI,     y1H[j],     y1He[j],     EH[j],     Te[j],     EHII[j],     THII[j],     EHI[j],     THI[j],     EHeI[j],     THeI[j],     dEH[j]\n");
  for(j=0; j<NGRID; j++) {
    printf("%4ld %11.5lE %8.15lf %8.6lf %8.6lf %7.15lf %8.6lf %7.15lf %8.6lf %7.15lf %8.6lf %7.15lf %8.10lf\n",
      j, (j+.5)*DNHI, y1H[j], y1He[j], EH[j], Te[j], EHII[j], THII[j], EHI[j], THI[j], EHeI[j], THeI[j], dEH[j]);
  }
